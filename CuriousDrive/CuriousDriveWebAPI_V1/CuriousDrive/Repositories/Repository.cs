...
namespace CuriousDriveWebAPI.CuriousDrive.Repositories
{
    public class Repository<TEntity> : IRepository<TEntity> where TEntity : class
    {
        ...
        {
            //InitializeEntities(new List<TEntity>() { entity });
        ...
        public void InitializeEntities(IEnumerable<TEntity> entities)
        {
            foreach (TEntity entity in entities)
            {
                ...
                {
                    if (propInfo.Name == "CreatedDate" || propInfo.Name == "ModifiedDate")
                        propInfo.SetValue(entity, DateTime.Now, null);

                    if (propInfo.Name == "UpdateSeq")
                        propInfo.SetValue(entity, 0, null);
                }
            }
        }
    }
